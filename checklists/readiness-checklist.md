...
Tick the boxes when each item is verified when running the compose stack.

- [x] DB: `pg_isready` returns OK
- [x] AI service: `/health` returns 200
- [x] API: `/health` returns 200 and shows AI reachable
- [x] Env: `.env` populated and no real secrets committed
- [x] Network: services can call each other via `team-internal` (e.g., `http://ai-service:9000`)
- [x] Version: images use tag prefix `v0.1.0-` or similar
